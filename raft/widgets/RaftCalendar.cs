...
public sealed class RaftCalendar : JustInTimeRenderable, IHasCulture, IHasTableBorder, IAlignable {
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="RaftCalendar" /> class.
    /// </summary>
    /// <param name="date">The calendar date.</param>
    public RaftCalendar(DateTime date)
    ...
    public RaftCalendar(int year, int month)
        : this(year, month, 1) {
    }
    ...
    public RaftCalendar(int year, int month, int day) {
        _year = year;
        _month = month;
        _day = day;
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        if (table.Rows.Count < ExpectedRowCount) {
            ...
            for (var i = 0; i < diff; i++) table.AddEmptyRow();
        }
    ...
    private DayOfWeek[] GetWeekdays() {
        ...
        for (var i = 1; i < 7; i++) days[i] = days[i - 1].GetNextWeekDay();
